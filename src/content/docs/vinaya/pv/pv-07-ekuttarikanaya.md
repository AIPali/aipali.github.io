...
> [!abstract] AI 导读
> 本篇名为《增一法》，是《附随》的第七章。其结构模仿了《增支部》（Aṅguttara Nikāya），将律藏中的各种概念、规则和程序，按照从一到十一的数量递增方式进行分类汇编。这种编排方式不仅便于记忆和检索，也体现了律藏内容的系统性和组织性。本章涵盖了从过患（罪）的分类、僧团的正式程序（羯磨），到对不同比丘资格的认定等方方面面的律学要点，是理解和掌握整个戒律体系的重要纲要。
